...
- [The exceptions registry](#the-exceptions-registry)
  - [The exceptions.ts file](#the-exceptionsts-file)
...
1. Reads your `package.json` scripts and `.husky/` hooks.
2. Detects your package manager (npm / pnpm / yarn).
3. Generates a starter `effective.config.ts` at the repo root.
4. Generates a starter `.effective/exceptions.ts`.
5. Adds `.effective/` to your `.gitignore`.
6. Pre-installs `node_modules` to `.effective/node_modules` so future
   verify calls are fast.
...
### The exceptions.ts file

Lives at `.effective/exceptions.ts`. Generated by `init`:
...
Step 4: commit your `effective.config.ts` and `.effective/exceptions.ts`.
...
Before you write your initial `.effective/exceptions.ts`, see what's already
in the codebase:
...
```ts
{
  ...
  message: '.skip on a test without an exception ref. Add a tracked exception in .effective/exceptions.ts and cite its ID in the comment, or fix the underlying test failure.',
  source: {
    kind: 'rule',
    ruleId: 'no-disabled-tests-without-exception',
  },
