...
/**
 * `.effective/` is engine workspace: the isolated worktree, the cached
 * `node_modules`, and (future) the parser cache. User-authored config
 * lives in `effective.config.{ts,js}` at the repo root, so the whole
 * `.effective/` tree can be gitignored without losing project state.
 */
const EFFECTIVE_GITIGNORE_ENTRIES = ['.effective/'];
  ...
  for (const p of written) {
    ...
    switch (rel) {
      case `effective.config.${configExt}`: {
        lines.push(
          `✓ Created ${rel} (extends recommended preset; toolchain detected; built-in exceptions ready)`,
        );
        break;
      }
      case '.gitignore': {
